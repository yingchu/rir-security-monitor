...
- **各自為政** — 五大 RIR 彼此獨立運作，跨 RIR 合併分析在技術與政治上都有高昂的協調成本。本工具預設就橫跨全部五個。
...
需要 Python 3.10 以上版本。不需要額外安裝系統工具——WHOIS 查詢改用各 RIR 的公開 RDAP 端點，透過 `requests` 直接存取。
...
產生報告時，`whois_enrich.py` 會自動對所有高風險預警進行登記資料查詢。針對每個標記的 `opaque_id`，流程如下：

1. 從 delegation 檔案找出對應的 IPv4 起始位址或 ASN
2. 依 delegation 記錄中的 registry 欄位，直接查詢對應 RIR 的公開 **RDAP** 端點（如 `rdap.apnic.net`、`rdap.db.ripe.net`）
3. 從結構化 JSON 回應中解析登記實體名稱、描述與國家
4. 在 HTML 報告頂部的摘要表格中呈現結果

RDAP 是取代傳統 WHOIS 文字查詢的現代標準，回傳結構化 JSON，不需要安裝系統 `whois` 工具，欄位解析在各 RIR 之間一致可靠。查詢結果仍應獨立核實——登記資料可能不完整或未即時更新。
